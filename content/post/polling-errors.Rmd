...
{{% callout note %}}
This post is from 2022, before the midterm elections. I 
have [an updated post](/post/polling-errors-map-24) that incorporates the 2022
midterms, uses slightly better models, and has generally better fine-tuning. 
I'd recommend checking that out if you're interested in the subject matter on
this page.
{{% /callout %}}
...
Finally, the **third map** lets you take the current (as of 11/6/22)
...
fig <-  plot_ly(dg) %>% add_trace(
  type = "choropleth", locations = ~name, featureidkey = "properties.NAME",
  geojson = usgeo,  z = ~bias98, color = ~bias98, 
  colorscale = list(c(0, 0.5, 1), c(muted("red"), "white", muted("blue"))),
  zmin = -8, zmax = 8,  hovertemplate = 'D %{z:+2.1d}%<extra>%{location}</extra>',
  colorbar = list(title = list(text = "Average Bias", side = "top"),
                  len = 150, lenmode = "pixels",
